...
const { Text, DateTime, Virtual, Checkbox } = require('@keystonejs/fields')
  ...
  fields: {
    ...
    clientPositions: {
      ...
      resolver: async (item, args, context) => {
        if (item.end && positionCache[item.id]) {
          return positionCache[item.id]
        }
        ...
        if (!errors) {
          ...
          positionCache[item.id] = positions
          return positionCache[item.id]
        }
        return []
      },
