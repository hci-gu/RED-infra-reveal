...
class WebSocketAdapter:
    ...
    def send_message(self, metadata, data1, data2):
        ...
        all_data = struct.unpack(
            "<II" + str(new_response_size - 8) + "s", new_response)
    ...
    def responseheaders(self, flow):
        ...
        flow.response.stream = flow.request.path not in self.intercept_paths and self.only_intercept_text_files and not is_text_response(
    ...
    async def websocket_loop(self):
        ...
        while not self.finished:
            try:
                ...
                ctx.log.error(
                    "[mitmproxy-websocket] ConnectionClosed error:", sys.exc_info())
                pass
            except BrokenPipeError:
                # Connect failed
                ctx.log.error(
                    "[mitmproxy-websocket] BrokenPipeError error:", sys.exc_info())
                pass
            except IOError:
                # disconnected from server mis-transfer
                ctx.log.error(
                    "[mitmproxy-websocket] IOError error:", sys.exc_info())
                pass
            except:
                ctx.log.error(
                    "[mitmproxy-websocket] Unexpected error:", sys.exc_info())
